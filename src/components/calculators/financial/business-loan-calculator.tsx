...
// Essentially the same logic as Loan Payment, but possibly with different term units or focus
...
import { Calculator, Star, Building, HandCoins, ListChecks } from 'lucide-react';
...
// Schema (similar to Loan Payment, maybe adjust term label/units)
const businessLoanSchema = z.object({
  ...
  loanTerm: z.string().refine(val => !isNaN(parseInt(val)) && parseInt(val) > 0, {
    message: "Loan term must be a positive integer (years).", // Or months? Adjust as needed
  }),
  // Optionally add fields like loan fees if relevant for business loans
});
...
export function BusinessLoanCalculator({ slug, calculatorInfo, onCalculation, favorites, setFavorites, currency }: BusinessLoanCalculatorProps) {
    ...
    // Calculation logic (identical to Loan Payment)
     const calculateLoanPayment = (values: BusinessLoanFormValues): { payment: number; totalInterest: number; totalPayment: number; schedule: AmortizationEntry[] } | null => {
         const principal = parseFloat(values.loanAmount);
         const annualInterestRate = parseFloat(values.interestRate) / 100;
         const years = parseInt(values.loanTerm); // Assuming term is in years
         ...
         const numberOfPayments = years * 12; // Assuming term is years, payments are monthly
    ...
    const onSubmit: SubmitHandler<BusinessLoanFormValues> = (data) => {
        ...
        if (result) {
            ...
            const inputString = `Loan Amount: ${formatCurrency(parseFloat(data.loanAmount))}, Rate: ${data.interestRate}%, Term: ${data.loanTerm} years`; // Adjust term unit if needed
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input type="number" placeholder="e.g., 8.5" {...field} step="any" min="0" />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                     <FormLabel>Loan Term (Years)</FormLabel> {/* Or change label/step if using months */}
                                    <FormControl>
                                         <Input type="number" placeholder="e.g., 5" {...field} step="1" min="1" /> {/* Step 1 for years */}
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        ...
                        <div className="md:col-span-3">
                            <Button type="submit" className="w-full">
                                <Calculator className="mr-2 h-4 w-4" /> Calculate Business Loan
                            </Button>
                        </div>
                    </form>
                </Form>
